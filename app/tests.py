...
class TestModels (TestCase) :
    ...
    def test_comic_query_1 (self) :
        comic = Comic.query.filter_by.filter_by(Comic.title=="Adam: Legend of the Blue Marvel (2008) #2").first()
    ...
    def test_comic_query_2 (self) :
        comic = Comic.query.filter_by.filter_by(Comic.title=="Adam: Legend of the Blue Marvel (2008) #1").first()
    ...
    def test_creator_query_1 (self) :
        creator = Creator.query.filter_by.filter_by(Creator.id=='2289').first()
        self.assertEqual(creator.first_name, Alexandrov)

    def test_creator_query_2 (self) :
        creator = Creator.query.filter_by.filter_by(Creator.first_name=='Alexandrov').first()
        self.assertEqual(creator.id, 2289)

    def test_creator_query3 (self) :
        creator = Creator.query.filter_by.filter_by(Creator.name=='Amash').first()
